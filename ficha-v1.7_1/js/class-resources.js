...
function renderRunimago(container) {
    ...
    /* --- BLOCO 1: REPERTÓRIO RÚNICO --- */
    const rep = document.createElement('div'); rep.className = 'runimago-block';
    rep.innerHTML = '<div class="runimago-block-title">📜 Repertório Rúnico</div>';
    const grid = document.createElement('div'); grid.className = 'repertorio-grid';

    // 1A — ARTUS
    const colA = document.createElement('div'); colA.className = 'repertorio-col';
    colA.innerHTML = '<div class="repertorio-col-title">Artus</div>';
    ARTUS.forEach(a => {
        const row = document.createElement('div'); row.className = 'sk-row';
        const lbl = document.createElement('div'); lbl.className = 'sk-label';
        lbl.innerHTML = `<div class="sk-name">${a.name}</div><div class="sk-attr">${a.desc}</div>`;
        row.appendChild(lbl); row.appendChild(createDotsHTML(a.key)); colA.appendChild(row);
    });
    grid.appendChild(colA);

    // 1B — ASPECTUS
    const colB = document.createElement('div'); colB.className = 'repertorio-col';
    colB.innerHTML = '<div class="repertorio-col-title">Aspectus</div>';
    ASPECTUS.forEach(a => {
        const row = document.createElement('div'); row.className = 'sk-row';
        const lbl = document.createElement('div'); lbl.className = 'sk-label';
        lbl.innerHTML = `<div class="sk-name">${a.name}</div>`;
        row.appendChild(lbl); row.appendChild(createDotsHTML(a.key)); colB.appendChild(row);
    });
    grid.appendChild(colB);

    // 1C — SIGILUS
    const colC = document.createElement('div'); colC.className = 'repertorio-col';
    colC.innerHTML = '<div class="repertorio-col-title">Sigilus</div><div id="sigilusContainer"></div>';
    const addSigBtn = document.createElement('button');
    addSigBtn.className = 'add-btn no-print'; addSigBtn.textContent = '+ Sigilus';
    addSigBtn.style.cssText = 'margin-top:4px;font-size:10px;padding:6px';
    addSigBtn.addEventListener('click', () => addSigilus());
    colC.appendChild(addSigBtn);
    ...
    // Load saved sigilus
    sigilusCount = 0;
    state.sigilus.forEach(s => addSigilus(s));

    /* --- BLOCO 2: RUNAS PREPARADAS --- */
    const rpBlk = document.createElement('div'); rpBlk.className = 'runimago-block';
    rpBlk.innerHTML = `<div class="runimago-block-title">⚡ Runas Preparadas</div><div id="runasPrepContainer"></div>`;
    const addRPBtn = document.createElement('button');
    addRPBtn.className = 'add-btn no-print'; addRPBtn.textContent = '+ Adicionar Runa Preparada';
    addRPBtn.addEventListener('click', () => addRunaPrep());
    rpBlk.appendChild(addRPBtn);
    container.appendChild(rpBlk);

    runaPrepCount = 0;
    state.runasPrep.forEach(r => addRunaPrep(r));

    /* --- BLOCO 3: GRIMÓRIO DE ESTUDO --- */
    const estBlk = document.createElement('div'); estBlk.className = 'runimago-block';
    estBlk.innerHTML = `<div class="runimago-block-title">📖 Grimório de Estudo</div><div class="estudo-header-info">Slots: <span class="estudo-slots" id="estudoSlots">0 / 2</span></div><div class="estudo-head"><span>Estudando</span><span>Tipo</span><span>Nível</span><span>Progresso</span><span></span></div><div id="estudosContainer"></div>`;
    const addEstBtn = document.createElement('button');
    addEstBtn.className = 'add-btn no-print'; addEstBtn.textContent = '+ Adicionar Estudo';
    addEstBtn.addEventListener('click', () => addEstudo());
    estBlk.appendChild(addEstBtn);
    container.appendChild(estBlk);

    estudoCount = 0;
    state.estudos.forEach(e => addEstudo(e));
    updateEstudoSlots();
}
