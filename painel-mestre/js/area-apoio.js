...
window.deleteApoio = async function(i) {
    ...
    const original = S.todosApoiosCarregados[i];
    if (!original) { showAlert('⚠️ Apoio não encontrado, recarregue a lista', 'warning'); return; }
    if (!await confirmar(`Excluir o apoio "${original.nome || 'sem nome'}" (x${original.montante || 1})?`, { perigo: true })) return;
    const chaveOriginal = chaveApoio(original);
    try {
        await mutateApoios(S.currentSelectedUserId, apoios => {
            const idx = apoios.findIndex(a => chaveApoio(a) === chaveOriginal);
...
window.remProd = async function(i) { if (await confirmar('Remover?', { perigo: true })) { S.listaProducao.splice(i, 1); await salvarProd(); renderProd(); } };
window.editProd = async function(i, f) { const v = await perguntar(`Editar ${f}:`, { valor: S.listaProducao[i][f] || '' }); if (v !== null) { S.listaProducao[i][f] = v; await salvarProd(); renderProd(); } };
...
window.deleteItemRepertorio = async function (userId, itemIndex) {
    if (!await confirmar('Tem certeza que deseja deletar este item do repertório?', { perigo: true })) return;
...
window.deleteLojaItem = async function(id) {
    if (!await confirmar('Tem certeza que deseja excluir este item? Essa ação não pode ser desfeita.', { perigo: true })) return;
    try {
